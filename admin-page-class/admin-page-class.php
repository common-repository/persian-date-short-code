  ...
  class BF_Admin_Page_Class {
    ...
    public function DisplayPage() {
      ...
      echo '<form method="post" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        ...
        echo '<h3>'.apply_filters('admin_page_class_h2',$this->args['page_title']).'</h3>'.((isset($this->args['page_header_text']))? $this->args['page_header_text'] : '').' 
        </div>
        <div style="float:left;margin:32px 0 0 0">
          <input type="submit" style="margin-left: 25px;" value="'.esc_attr(__('ذخیره تغییرات','apc')).'" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn"><br><br>
        </div>
      ...
      if ($this->saved_flag){
        ...
        if (is_array($this->errors) && count($this->errors) > 0 ){
          ...
          echo '<div class="alert alert-success"><button data-dismiss="alert" class="close" type="button">×</button><strong>'.__('تنظیمات با موفقیت ذخیره شدند .','apc').'</strong></div>';
        }
        echo '</div>';
      }
    ...
    echo '</div><div style="clear:both"></div><div class="footer_wrap">
        <div style="float:left;margin:32px 0 0 0">
          <input type="submit" style="margin-left: 25px;" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn" value="'.esc_attr(__('ذخیره تغییرات','apc')).'" />
          <br><br>
        </div>
        <br style="clear:both"><br>
      </div>';
